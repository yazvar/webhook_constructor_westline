...
### Бот для выбора канала (опционально)

Чтобы в приложении работал **выбор сервера и канала** (а не только ручной ввод URL вебхука):

1. В том же приложении → вкладка **Bot** → **Add Bot**, скопируй **Token** (`DISCORD_BOT_TOKEN`).
2. Бот должен иметь право **Manage Webhooks** (и **View Channel**) в нужных каналах.
3. Пригласи бота на сервер(ы): вкладка **OAuth2 → URL Generator**, scope `bot`,
   право `Manage Webhooks`, открой получившуюся ссылку и добавь бота.

Приложение через бота показывает список серверов и текстовых каналов, а при отправке
создаёт/переиспользует вебхук `Westline` в выбранном канале. Если `DISCORD_BOT_TOKEN`
не задан, выбор канала скрыт и остаётся ручной ввод URL вебхука.
...
| GET | `/api/discord/guilds` | сессия | серверы бота (для выбора канала) |
| GET | `/api/discord/guilds/:id/channels` | сессия | текстовые каналы сервера |
| POST | `/api/discord/resolve-webhook` | сессия | URL вебхука для канала (создаёт при необходимости) |
